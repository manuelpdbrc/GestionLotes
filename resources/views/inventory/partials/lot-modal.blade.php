    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div class="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6"
            ...
            <div x-show="!loadingData && selectedLot" style="display: none;">
                ...
                <div x-show="isSupervisor" class="mt-4 border-t pt-4 space-y-4">
                    ...
                    <div x-data="{ extendTime: '', showExtend: false }" x-show="selectedLot?.block" class="bg-yellow-50 border border-yellow-200 p-3 rounded-md">
                        <div class="grid grid-cols-2 gap-2">
                            <button type="button" @click="showExtend = !showExtend" class="w-full justify-center rounded-md border border-yellow-300 px-4 py-2 bg-yellow-100 text-yellow-800 hover:bg-yellow-200 text-sm">
                                Extender
                            </button>
                            <button type="button" 
                                @click="if(confirm('Liberar bloqueo manualmente?')) {
                                    ...
                                    .then(res => { alert(res.message); window.location.reload(); });
                                }" class="w-full justify-center rounded-md border border-red-300 px-4 py-2 bg-red-100 text-red-800 hover:bg-red-200 text-sm">
                                Liberar Bloqueo
                            </button>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 pt-2">
                        <button type="button" 
                                x-show="selectedLot?.estado !== 'reservado' && selectedLot?.estado !== 'vendido'"
                                @click="if(confirm('Marcar como reservado?')) {
                                    window.fetchApi('/lots/' + selectedLot.id + '/reserve', { method: 'PUT' })
                                    .then(res => { alert(res.message); window.location.reload(); });
                                }"
                                class="w-full justify-center rounded-md border border-transparent px-4 py-2 bg-gray-800 text-white hover:bg-gray-900 flex focus:outline-none sm:text-sm">
                            Pasar a Reserva
                        </button>
                        <button type="button" 
                                x-show="selectedLot?.estado !== 'vendido'"
                                @click="if(confirm('Marcar como vendido?')) {
                                    window.fetchApi('/lots/' + selectedLot.id + '/sell', { method: 'PUT' })
                                    .then(res => { alert(res.message); window.location.reload(); });
                                }"
                                class="w-full justify-center rounded-md border border-transparent px-4 py-2 bg-gray-900 flex text-white hover:bg-black focus:outline-none sm:text-sm"
                                :class="{'col-span-2': selectedLot?.estado === 'reservado'}">
                            Confirmar Venta
                        </button>

                        <button type="button" 
                                x-show="selectedLot?.estado === 'reservado' || (selectedLot?.estado === 'bloqueado' && !selectedLot?.block)"
                                @click="if(confirm('Devolver a Disponible?')) {
                                    window.fetchApi('/lots/' + selectedLot.id + '/revert', { method: 'PUT' })
                                    .then(res => { alert(res.message); window.location.reload(); });
                                }"
                                class="w-full col-span-2 justify-center rounded-md border border-gray-300 px-4 py-2 flex bg-white text-gray-700 hover:bg-gray-50 focus:outline-none sm:text-sm">
                            Liberar Lote (Volver Disponible)
                        </button>
                    </div>
                </div>
                ...
                <!-- Historial de Lote Expandible -->
                <div x-data="{ expandedHistory: false, historyData: [], loadingHistory: false }" class="mt-4 border-t border-gray-200">
                    <button type="button" @click="
                        expandedHistory = !expandedHistory;
                        if(expandedHistory && historyData.length === 0) {
                            loadingHistory = true;
                            fetch('/api/lots/' + selectedLot.id + '/history')
                                .then(r => r.json())
                                .then(d => { historyData = d; loadingHistory = false; })
                                .catch(e => { alert('Error cargando historial'); loadingHistory = false; });
                        }
                    " class="flex w-full items-center justify-between py-3 text-sm font-medium text-gray-500 hover:text-gray-700">
                        ...
                        <svg class="h-5 w-5 transform transition-transform" :class="{'rotate-180': expandedHistory}" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                        </svg>
                    </button>

                    <div x-show="expandedHistory" x-collapse class="bg-gray-50 rounded-b-lg overflow-hidden">
                        <div x-show="loadingHistory" class="p-4 flex justify-center text-gray-400">
                            <span class="animate-pulse">Cargando...</span>
                        </div>
                        <div x-show="!loadingHistory && historyData.length === 0" class="p-4 text-center text-sm text-gray-500">
                            No hay registros de cambios para este lote.
                        </div>
                        <ul x-show="!loadingHistory && historyData.length > 0" class="divide-y divide-gray-200">
                            <template x-for="item in historyData" :key="item.date + item.action">
                                <li class="p-4 text-xs sm:text-sm">
                                    <div class="flex justify-between items-start">
                                        <div class="flex flex-col">
                                            <span class="font-semibold text-gray-900" x-text="item.action"></span>
                                            <span class="text-gray-500" x-text="'Por: '+item.user"></span>
                                        </div>
                                        <div class="text-right flex flex-col">
                                            ...
                                            <span class="mt-1 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800" 
                                                  x-text="item.old_state ? (item.old_state + ' → ' + item.new_state) : item.new_state"></span>
                                        </div>
                                    </div>
                                </li>
                            </template>
